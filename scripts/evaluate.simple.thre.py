...
DEFINE_float('thre', -1, 'thre for desciding predict')
DEFINE_float('fpr', 0.002, 'the max fpr we can accept')
...
def find_thre(label_list, predicts):
	l = []
	total = len(label_list)
	for i in xrange(total):
		l.append((predicts[i], label_list[i]))
	l.sort(reverse = True)
	
	fp_count = int(total * FLAGS.fpr) 
	count = 0
...
def parse_input(input):
	...
	try:
		...
		if FLAGS.thre > -1:
			...
			thre = find_thre(label_list, predicts)
		print "Fpr thre: %.4f"%FLAGS.fpr
		print "Final thre: %.4f"%thre
...
def main(argv):
	...
	pos = len(argv) - 1
	try:
		FLAGS.fpr = float(argv[-1])
		pos -= 1
	except Exception:
		pass
	#---------------------------------thre
	print "Input thre: %.4f"%FLAGS.thre
	#---------------------------------deal input
	l = []
	if (FLAGS.regex != ""):
