...
namespace gezi {
  ...
  public:
    ...
    virtual Fvec FinishGroup()
    {
      ...
      { //@CHECK TLC 是   Double ndcg = DCGAtN[t] > 0 ? DCGAtN[t] / maxDCGAtN[t] * 100 : 0; //same as below
    ...
    virtual void Finish() override
    {
      ...
      if (sumWeights > 0)
      {
        _results[index++] = sumTop1Precision / sumWeights * 100;
      }

      if (sumWeights2 > 0)
      {
        _results[index++] = sumTop1Precision2 / sumWeights2 * 100;
      }
  ...
  public:
      ...
      for (auto& group : instanceGroups)
      {
        ...
        //@TODO 可以每个组并行处理也许
        {
          ...
          for (auto& instance : groupInstances)
          {
            ...
            rankingOutputs.emplace_back(move(perInstanceOutputs));
          }
        }
    ...
    virtual void Test_(istream& ifs) override
    {
      string line;
      string preQuery = "";
      vector<dvec> rankingOutputs;
      ...
      {
        string query = ProcessRankLine(line, preQuery);
        preQuery = query;
      }
      if (!preQuery.empty())
      {
        ...
        {
          static_pointer_cast<RankingMetrics>(datasetMetric)->FinishGroup();
        }
      }
    }

    //assume input line format is 
    //query\tdoc\tlabel\tprediction 
    //or
    //query\tdoc\tlabel\tprediction\tweight
    string ProcessRankLine(string line, string preQuery)
    {
      auto results = split(line, '\t');
      CHECK_GE(results.size(), 4);
      string query = results[0];
      string doc = results[1];
      Float label = FLOAT(results[2]);
      Float prediction = FLOAT(results[3]);

      if (!preQuery.empty() && query != preQuery)
      {
        for (auto& datasetMetric : datasetMetrics)
        {
          static_pointer_cast<RankingMetrics>(datasetMetric)->FinishGroup();
      ...
      Float weight = results.size() > 4 ? FLOAT(results[4]) : 1.0;

      for (auto& datasetMetric : datasetMetrics)
      {
        datasetMetric->ProcessInstance(label, prediction, prediction, weight);
      }
